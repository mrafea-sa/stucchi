...
{
    public class IDChartCalloutModel : _BaseModel
    {
        /// <inheritdoc />
        ...
        {
            var viewModel = new IDChartCalloutViewModel()
            {
                Title = this.Title,
        ...
        private ArrayList GetProcessedItemsSelectedItems(dynamic items)
        {
            ...
            foreach (dynamic item in items)
            {
                ...
                DynamicContent itemContent = DynamicContentOps.GetGenericItemById(ModuleTypes.CalloutsIdChartName, itemId);
