...
{
    public class ProductImageCalloutModel : _BaseModel
    {
        /// <inheritdoc />
        ...
        {
            var viewModel = new ProductImageCalloutViewModel()
            {
                Title = this.Title,
        ...
        private ArrayList GetProcessedItemsSelectedItems(dynamic items)
        {
            ...
            foreach (dynamic item in items)
            {
                ...
                DynamicContent itemContent = DynamicContentOps.GetGenericItemById(ModuleTypes.CalloutsProductImageName, itemId);

                dynamic itemToBeAdded = new ExpandoObject();

            itemToBeAdded.DisplayTitle = itemContent.GetValue<Lstring>("DisplayTitle");
            itemToBeAdded.ButtonText = itemContent.GetValue<Lstring>("ButtonText");
